...
final = pd.concat([ph] + list_df, ignore_index=True)  # concatenate all the dataframes together
...
print(f"Here is the latest cybersecurity updates\nDate: {current_date}\nTime: {current_time}")
print("Have a nice day.")

# Apply HTML formatting to 'Title' column if needed
final['Title'] = [f'<span class="title-text">{x}</span>' if x.strip() else x for x in final['Title']]

# Create the HTML content
finalstyle = pd.concat([ph, pm, rm, sdlc, gc, gc2, gc3, fbi_news, exfil, pod]).reset_index(drop=True)
html_content = finalstyle.to_html(escape=False, index=False)  # escape=False to render HTML links properly

# Assign the generated HTML content to 'final_html'
final_html = f"""
<html>
<head>
...
<body>
    <h1>Latest Cybersecurity Updates</h1>
